...
class BiLSTM_CRF(nn.Module):
    ...
    def _score_sentence(self, feats, tags):
        ...
        print(type(tags))
        print(type(torch.LongTensor([self.tag_to_ix[START_TAG]])))
        raise KeyboardInterrupt
...
training_data = [(np.random.randn(100, 100) * 3,
                  np.random.choice(['B', 'I', 'O'], size=100)),
                 (np.random.randn(123, 100) * 3,
                  np.random.choice(['B', 'I', 'O'], size=123))]
...
model = BiLSTM_CRF(10, tag_to_ix, EMBEDDING_DIM, HIDDEN_DIM)
optimizer = optim.SGD(model.parameters(), lr=0.01, weight_decay=1e-4)

# Check predictions before training
# precheck_sent = prepare_sequence(training_data[0][0], word_to_ix)
# precheck_tags = torch.LongTensor([tag_to_ix[t] for t in training_data[0][1]])
# precheck_sent = autograd.Variable(torch.from_numpy(training_data[0][0]).float())
...
for epoch in tqdm(range(10)):  # again, normally you would NOT do 300 epochs, it is toy data
    for sentence, tags in training_data:
        ...
        sentence_in = autograd.Variable(torch.from_numpy(sentence).float())
        # print(sentence_in)s
        # raise KeyboardInterrupt
